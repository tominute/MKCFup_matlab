function [distance_precision, PASCAL_precision, average_center_location_error,Overlap, flag] = ...
    compute_performance_measures(positions, ground_truth, distance_precision_threshold, PASCAL_threshold, tran)
...
%calculate trans
flag = 0;
for frame = 2:length(ground_truth)
    trans = sqrt((ground_truth(frame, 1) - ground_truth(frame-1, 1))^2 + (ground_truth(frame, 2) - ground_truth(frame-1, 2))^2);
    if trans > sqrt((ground_truth(frame, 3))^2 + (ground_truth(frame, 4))^2)*2.5/5
        flag = 1;
    end
end
